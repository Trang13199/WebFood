...
namespace Web_food.DAO
{
    ...
    {
        public void doRegister(string username, string password, string email, string phone, string address,string gender,int level)
        {
            DBConnection dbConnection = new DBConnection();
            MySqlConnection conn = dbConnection.ConnectionSql();
            conn.Open();
            
            string sql = "INSERT INTO user (username, `password`, email, phone, address,gender,level) VALUES(@username,@password,@email,@phone, @address,@gender,@level)";
        ...
        public bool checkUserExist(string account, string email)
        {
            ...
            string sqlselect = "select email from user";
